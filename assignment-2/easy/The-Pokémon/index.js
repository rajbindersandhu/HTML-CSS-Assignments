...
function addDropdown(){
    ...
    for(let i=0;i<data.results.length;i++){
        const type = data.results[i].name
        const optionNode = document.createElement("option");
        optionNode.setAttribute("value", type);
        optionNode.innerText = type;
...
async function fetchPokemonData(){
    ...
    if(pokeDataBase.length == 0){
        const res = await fetch("https://pokeapi.co/api/v2/pokemon?limit=1350");
        const data= await res.json();
        pokeDataBase = [...data.results];
    }
    
    // console.log(pokeList)
    // for(let i=0;i<pokeList.length;i++){
    //     // console.log(pokeList[i].url)
    //     const res2 = await fetch(pokeList[i].url);
    //     const data2 = await res2.json();
    //     for(let j=0;j<data2.types.length;j++){
    //         if(data2.types[j].type.name in pokeDataBase){
    //             pokeDataBase[data2.types[j].type.name].push(data2)
    //         }else{
    //             pokeDataBase[data2.types[j].type.name] = [data2]
    //         }
    //     }
        
    // }
    console.log("Pokemon: ", pokeDataBase);
}

let userList = [];

function createCard(pokeObjt){
    console.log("creating card")
    const card = document.createElement("div");
    card.setAttribute("class", "card");
    const nameTag = document.createElement("p");
    nameTag.setAttribute("class", "poke-name");
    nameTag.innerText = `${pokeObjt.name.charAt(0).toUpperCase()+pokeObjt.name.slice(1)} | ${pokeObjt.base_experience}`;
    card.appendChild(nameTag)
    const pokeImage = document.createElement("img");
    pokeImage.setAttribute("src", pokeObjt.sprites.other["official-artwork"]["front_default"])
...
function clearAllCards(){
    ...
    console.log("cards in container: "+container.children)
    if(count > 0){
        while(document.getElementById("card-container").children.length>0){
...
function fillTheContainer(cardList){
    console.log("filling container")
    clearAllCards();
    for(let i=0;i<cardList.length;i++){
        const card = createCard(cardList[i]);
...
async function handelClick(){
    ...
    console.log("number of cards requested "+numberOfCards);
    if(userList.length!=0) userList = []
    for(let i=0;i<numberOfCards;i++){
        const randomPokemon = pokeDataBase[(Math.floor(Math.random()*pokeDataBase.length))];
        const res = await fetch(randomPokemon.url);
        const data = await res.json();
        userList.push(data)
    }
    console.log("User list: "+userList);
    fillTheContainer(userList);
}


// addDropdown();
fetchPokemonData();
document.getElementById("cardBtn").addEventListener("click", handelClick);